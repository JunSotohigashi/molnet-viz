...
def extract_molecules(eq_path: str, ts_path: str):
    # ファイルの読み込み
    ...
    node_df1 = eq_df
    node_df2 = ts_df
    mol_df = pd.concat([node_df1, node_df2])
    return mol_df


def mol_df_to_xyz_blocks(mol_df: pd.DataFrame) -> list:
    xyz_list = []
    for _, mol in mol_df.iterrows():
        n = 0
        lines = [mol["id"]]
        while True:
            try:
                if mol["element" + str(n)] == "":
                    break
                lines.append(
                    f"{mol['element'+str(n)]} {mol['element'+str(n)+'_0']:0.10f} {mol['element'+str(n)+'_1']:0.10f} {mol['element'+str(n)+'_2']:0.10f}"
                )
                n += 1
            except KeyError:
                break
...
if __name__ == "__main__":
    mol_df = extract_molecules(EQ_path, TS_path)
    xyz_blocks = mol_df_to_xyz_blocks(mol_df)
    ...
    for xyz_block in xyz_blocks[:]:
        ...
        mol = rdkit.Chem.RWMol(mol)
        # print(mol.GetAtoms()[0].GetSymbol(), mol.GetConformer().GetAtomPosition(0)[0], mol.GetNumAtoms())

        n_atoms = mol.GetNumAtoms()
        distance_matrix = rdkit.Chem.rdmolops.Get3DDistanceMatrix(mol)

        TOLERANCE = 1.1
        for x in range(n_atoms):
            for y in range(x+1, n_atoms):
                pair = set([mol.GetAtoms()[x].GetSymbol(), mol.GetAtoms()[y].GetSymbol()])
                for bond in bond_db:
                    if pair == set(bond["elements"]) and distance_matrix[x,y] <= bond["length"]*TOLERANCE:
                        # print(pair, distance_matrix[x,y], distance_matrix[x,y]/bond["length"])
                        if bond["bond_order"] == 1:
                            mol.AddBond(x, y, rdkit.Chem.BondType.SINGLE)
                        elif bond["bond_order"] == 2:
                            mol.AddBond(x, y, rdkit.Chem.BondType.DOUBLE)
                        elif bond["bond_order"] == 3:
                            mol.AddBond(x, y, rdkit.Chem.BondType.TRIPLE)
                        break
        print(xyz_block)
        # print(rdkit.Chem.MolToXYZBlock(mol))
