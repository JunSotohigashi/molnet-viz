...
def arrange_nodes_by_smiles(G, grid_size=(5, 6), spacing=300):
    ...
    grid_x, grid_y = np.meshgrid(
        np.linspace(0, spacing * (grid_size[0] - 1), grid_size[0]),
        np.linspace(0, spacing * (grid_size[1] - 1), grid_size[1]),
    )
    grid_positions = list(zip(grid_x.flatten(), grid_y.flatten()))

    # クラスタの中心座標を割り当てる
    cluster_positions = {}
    for i, (smiles, _) in enumerate(clusters.items()):
        cluster_positions[smiles] = grid_positions[i]
    ...
    for smiles, cluster_nodes in clusters.items():
        ...
        cx, cy = cluster_positions[smiles]  # グリッド上のクラスタ中心座標
        for node, (x, y) in circular_pos.items():
            layout[node] = (cx + x, cy + y)  # クラスタ中心をオフセット

    # 座標情報をnetworkxのノード属性にセット
